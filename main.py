...
try:
    ...
    try:
        ...
        print("You have exhausted the questions. Progress has been deleted. Please restart app")
...
def restart_game():
    global to_learn
    os.remove("data/to_learn.csv")
    starting_data = pd.read_csv("data/python_questions.csv")
...
def next_card():
    ...
    try:
        ...
        canvas.itemconfig(question_answer, text="You've finished\nData has been deleted\nThe program will start "
                                                "from its initial state", fill="black", justify="center")
        window.after(DELAY, func=restart_game)
    else:
        canvas.itemconfig(canvas_background, image=front_image)
